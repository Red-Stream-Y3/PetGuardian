...
const ServiceBooking = ({ navigation, route }) => {
  ...
  const handleHirePress = async () => {
    ...
    const reqData = {
      ...
      endTime: allDay
        ? new Date(
            input.startDateTime.toISOString().split('T')[0] + ' 23:59:59'
          )
            .toISOString()
            .split('T')[1]
    ...
    >
      ...
      >
              ...
              children: (
                <Ionicons
                  ...
                  onPress={() => {
                    setInput({
                      ...input,
                      pets: input.pets.filter((p) => p._id !== pet._id),
                    });
                  }}
                />
              ),
        ...
        {bookingType === BOOKING_TYPES[1] && (
          <Animated.View
            entering={
              prevType === BOOKING_TYPES[2] ? SlideInLeft : SlideInRight
            }
            exiting={
              prevType === BOOKING_TYPES[0] ? SlideOutLeft : SlideOutRight
            }
          >
            <DailyBooking
              styles={styles}
        ...
        <Text style={styles.textBody}>
          ...
          <Text style={{ fontWeight: 'bold' }}>
            {calculateFees(
              ...
              bookingType,
              allDay,
              service.services.fees.find((fee) => fee.tag === bookingType)
                .price,
              continuous,
              oneDay
            )}
