...
const UpdateRequest = ({ route }) => {
  ...
  useEffect(() => {
    const fetchRequest = async () => {
      const data = await getJoinRequestById(id, requestId);
      if (data) {
        console.log('data', data);
        setSelectedPetIds(data.pets[0]);
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await updateJoinRequest(id, requestId, request);
      console.log('response', response);
      if (response) {
        navigation.goBack();
      }
    } catch (error) {
      console.log('Error updating join request', error);
    }
  };
