...
const UpdatePlaydate = ({ route }) => {
  ...
  useEffect(() => {
    // Fetch playdate data by ID from the route.params.playdateId and populate the state variables
    const fetchPlaydateData = async () => {
      try {
        ...
        setSelectedPetIds(data.pets[0]._id);
        setDate(new Date(data.date));
        setTime(new Date(data.time));
        setLocation(data.location);
        setSpecialNotes(data.description);
        setLoading(false);
      } catch (error) {
        console.log('Error fetching playdate:', error);
      }
    };
  ...
  const handleUpdate = async () => {
    try {
      ...
      console.log('id', id);
      const updatedPlaydateData = await updatePlayDate(id, playdateData);
      if (updatedPlaydateData) {
        navigation.goBack();
      } else {
        console.log('Error updating playdates');
      }
    } catch (error) {
      console.log('Error updating playdate:', error);
    }
  };
