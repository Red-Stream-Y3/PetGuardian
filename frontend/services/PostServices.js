...
const BASE_URL = 'http://192.168.1.3:9120';
...
export const updatePost = async (post, token) => {
    ...
    try {
        const response = await axios.put(`${BASE_URL}/api/v1/posts/${post._id}`, post, config);
        return response.data;
    } catch (error) {
...
export const deletePost = async (id, token) => {
    ...
    try {
        const response = await axios.delete(`${BASE_URL}/api/v1/posts/${id}`, config);
        return response.data;
    } catch (error) {
...
export const getPostByUser = async (id, token) => {
    ...
    try {
        const response = await axios.get(`${BASE_URL}/api/v1/posts/user/${id}`, config);
        return response.data;
    } catch (error) {
        throw new Error('Error fetching posts');
    }
